...
func ItemSaverCocosLocal(index string) (chan engine.Item, error) {
	...
	//h := ReadTemplateLocal(s)
	//cpp := ReadTemplateLocal(ss)
...
func GenerateFileLocal(classes map[string][]model.Memitem, includeprefix string, h string, cpp string) {
...
//读取模板
func ReadTemplateLocal(name string) string {
...
func CreateFileLocal(packageName string, classname string, content string) {
